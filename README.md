...
- **Toolbar**: "GoSend" title (blue), vertical separator, then **Transfer Queue**, **Discover**, and (right-aligned) **Settings**. Toolbar background uses the theme mantle color, with a thin (1px) divider line below it.
- **Window frame spacing**: The main window disables Fyne's default window content padding so the app renders edge-to-edge against the window content area.
- **Left pane**: Peers list with "PEERS" header, compact refresh icon button, and online-count badge. No progress bar in the list; transfer state is text only ("Transferring... N%"). Rows show custom peer names when set, original device name as secondary text, `[Trusted]` / `[Verified]` badges, and a blue left-edge indicator when the peer is selected. Status dot colors: green (online), yellow (connecting/reconnecting), gray (offline).
- **Right pane**: Chat with selected peer. **Chat header**: compact single-line row showing peer name and truncated peer ID inline, plus compact search and peer-settings icon buttons; background mantle. **Search bar** (toggle): search icon, search input, "Files only" checkbox, and clear button on a compact surface row. A thin (1px) divider line separates the selected-peer header area from the transcript. **Transcript**: text messages render as lightweight rows (sender, timestamp, delivery mark) while file transfers render as surface cards with direction label, filename, metadata, progress bar, optional path, and compact action links (Cancel, Retry, Open File, Copy Path) shown only when applicable for the transfer state. Message rows and file cards now include a hover highlight state for clearer interactivity. **Composer**: mantle background with compact attach/send icons and a bordered message input.
- **Status bar**: Darkest (crust) background with compact height and padding; smaller status text on the left; compact **Logs** button on the right opens the **Application Logs** dialog (Runtime tab: last 500 status lines; Security Events tab: recent security events from the store). A thin (1px) divider line separates content from the footer.
- **Modal panel chrome**: Transfer Queue, Discover, Device Settings, Peer Settings, and Application Logs now share the same framed modal style (mantle header + compact top-right `X`, thin divider lines, consistent footer action button sizing, and edge-to-edge popup padding).
- **Transfer Queue** dialog: Mockup-aligned header/subtitle and close icon, filter tabs (All, Active, Completed, Issues), per-transfer rows (direction icon, filename, size, peer/status/speed/ETA, status-colored progress bar, optional stored path, compact action controls), and footer with completion/issue counts plus Clear Completed / Close.
- **Discover** dialog: Edge-to-edge styled panel (no extra dialog chrome padding) with custom header (title/subtitle + close icon), selectable rows (wifi/wifi-off status icon, name with fallback short ID, `added` badge, address:port, online/offline line), and compact footer actions: Refresh, Add Selected, and Close.
- **Device / Peer settings**: Presented in the same modal panel chrome with header close icon and compact footer `Close`/`Save` actions. Help text for max file size (0 = unlimited globally; 0 = use global default per-peer), Trust Level (informational badge), and Verified out-of-band (manual fingerprint comparison). Single-file attach flow (no "Add another file" prompt).
...
Key flows: Add peer (discovery → select → dial → `peer_add_request`); incoming add (accept/reject). Message (Enter to send); files (multi-file picker → queued `SendFile`), folders (drag/drop directory → `SendFolder` manifest + queued files), transfer queue management (top-bar queue panel with filters, cancel/retry, and show-path action), key change (trust/reject with fingerprints). Incoming file (auto-accept or accept/reject with name/size/type based on per-peer/global settings). Peer settings (chat-header gear → save custom name/trust/transfer policy).
